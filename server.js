...
// Verify JWT + check user exists in DB
...
// Route: Get current user's watch history (with ratings when available)
app.get('/api/dashboard/watch-history', authenticateToken, async (req, res) => {
    try {
        ...
        const watchHistory = [...fromWh, ...fromRatings].sort((a, b) => new Date(b.watched_at) - new Date(a.watched_at)).slice(0, 50);
...
// Route: Add to watch history (also marks as completed in watch_status)
...
//////////////////////////////////////////////////////
// WATCH STATUS (User Story) — GET + POST (UPSERT)
//////////////////////////////////////////////////////

// GET all statuses for current user (syncs watch_history → completed first)
...
// UPSERT status (prevents duplicates by UNIQUE key)
...
// TMDB: Watch providers for a movie or show
...
// Unread counts (needs message.read_at — run friends_message_read_migration.sql or npm run migrate:friends)
// Optional ?from=friend@email = unread in that thread only.
...
// One query: unread counts grouped by sender (for per-friend UI badges)
app.get('/api/friends/messages/unread/summary', authenticateToken, async (req, res) => {
    let connection;
    try {
        connection = await createConnection();
        const [rows] = await connection.execute(
            `SELECT sender_email AS senderEmail, COUNT(*) AS \`count\`
             FROM message
...
app.put('/api/friends/:email/messages/read', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (isUnknownColumnError(error)) {
            return res.status(200).json({ marked: 0, note: 'read_at missing; run migrate:friends' });
        }
        console.error(error);
